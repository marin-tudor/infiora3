...
import type { IBookingSettings } from '@/types'
...
export const bookingApi = createApi({
  ...
  endpoints: builder => ({
    ...
    getTimeSlots: builder.query<ITimeSlot[], { hotelId: string; itemId?: string; from?: string; to?: string }>({
      query({ hotelId, ...params }) {
    ...
    getBookingSettings: builder.query<IBookingSettings, string>({
      query(hotelId) {
        return { url: `/v1/hotels/${hotelId}/bookings/settings`, credentials: 'include' }
      },
      providesTags: [{ type: 'Booking', id: 'SETTINGS' }]
    }),
    updateBookingSettings: builder.mutation<IBookingSettings, { hotelId: string } & IBookingSettings>({
      query({ hotelId, ...body }) {
        return { url: `/v1/hotels/${hotelId}/bookings/settings`, method: 'PATCH', body, credentials: 'include' }
      },
      invalidatesTags: [{ type: 'Booking', id: 'SETTINGS' }]
    })
  })
})
