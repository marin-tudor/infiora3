...
const InsightsPage = () => {
  ...
  const setTab = (tab: string) => {
    router.push(`/${locale}/insights?` + toSearchParams({ ...searchParams, tab }))
  }

  const setRange = ({ startDate, endDate }: { startDate?: string; endDate?: string }) => {
    router.push(`/${locale}/insights?` + toSearchParams({ ...searchParams, startDate, endDate }))
  }
  ...
  const languageOptions = [
    ...new Set((insights?.activities || []).map(a => normalizeLanguageLabel(a.details.language)))
  ].map(language => ({
    label: language,
    value: language
  }))
      ...
      <Stack gap={3}>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ sm: 'center' }} gap={2}>
            <InsightsFilter
              ...
              setFilters={filters => {
                ...
                router.push(`/${locale}/insights?` + toSearchParams({ ...rest, ...filters }))
              }}
            />
            <RangePicker
        ...
        <TabContext value={activeTab}>
          ...
          >
            ...
            />
            <Tab
              icon={<i className='ri-hotel-bed-line' style={{ fontSize: 18 }} />}
              iconPosition='start'
              label='Operations'
              value='operations'
            />
          ...
          <TabPanel value='operations' sx={{ px: 0 }}>
            {authUser?.hotel?.id && <OperationsOverviewTab hotelId={authUser.hotel.id} />}
          </TabPanel>
