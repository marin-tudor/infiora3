...
const resourceId = (resource: Resource) => resource._id ?? resource.id ?? ''
...
export default function ResourceCalendar({ hotelId, selectedDate, slots }: Props) {
  ...
  const fetchResources = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API}/api/v1/hotels/${hotelId}/bookings/resources`, {
        credentials: 'include'
      })
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API}/api/v1/hotels/${hotelId}/bookings?date=${selectedDate}&limit=100`, {
        credentials: 'include'
      })
  ...
  const uniqueStartTimes = Array.from(new Set(slots.map(slot => slot.startTime))).sort()

  const formatTime = (iso: string) =>
    new Date(iso).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })

  const getCell = (resource: Resource, startTime: string) => {
    const slot = slots.find(value => value.startTime === startTime)

    if (!slot) {
      return { kind: 'no-slot' as const }
    }

    const booking = bookings.find(
      value => value.startTime === startTime && value.assignedResourceId === resourceId(resource)
    )

    if (booking) {
      return { kind: 'booked' as const, booking, slot }
    }

    if (slot.isBlocked) {
      return { kind: 'blocked' as const, slot }
    }

    return { kind: 'available' as const, slot }
  }

  const handleAssign = async (bookingId: string, assignedResourceId: string) => {
    setAssigning(true)

    try {
      const response = await fetch(`${API}/api/v1/hotels/${hotelId}/bookings/${bookingId}`, {
        method: 'PATCH',
        credentials: 'include',
        headers: createCsrfHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ assignedResourceId })
      })
  ...
  return (
    <Box sx={{ overflowX: 'auto' }}>
      ...
      <Table size='small' sx={{ tableLayout: 'fixed', minWidth: 600 }}>
        <TableHead>
          <TableRow>
            <TableCell sx={{ fontWeight: 700, minWidth: 140, bgcolor: 'background.paper' }}>Resource</TableCell>
        ...
        <TableBody>
          {resources.map(resource => (
            <TableRow key={resource._id} hover>
              ...
              {uniqueStartTimes.map(startTime => {
                const cell = getCell(resource, startTime)
                ...
                return (
                  <Tooltip
                    key={startTime}
                    title={`${itemName(booking.itemId)} - Room ${booking.guestRoomNumber} (${booking.bookingRef})`}
                  >
                    <TableCell
                      align='center'
                      onClick={event =>
                        setPopover({ anchor: event.currentTarget, booking, resourceId: resourceId(resource) })
                      }
                      sx={{
        ...
        >
          <Box p={2} minWidth={220}>
            ...
            <Typography variant='body2'>
              Start:{' '}
              {new Date(popover.booking.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
            </Typography>
