...
export default function ItemAvailabilityCalendar({ hotelId, itemId, weekStart }: Props) {
  ...
  const {
    ...
    isFetching,
    refetch
  } = useGetTimeSlotsQuery({ hotelId, itemId, from: weekStart, to: weekEnd }, { skip: !hotelId || !itemId })
  ...
  const allTimes = Array.from(new Set(slots.map(slot => slot.startTime.slice(11, 16)))).sort()
              ...
              return (
                <th
                  ...
                  style={{
                    ...
                    background: isToday ? 'rgba(25,118,210,0.06)' : undefined
                  }}
                >
                  <Typography
                    variant='caption'
        ...
        <tbody>
          {allTimes.map(time => (
            <tr key={time}>
              ...
              {days.map(day => {
                const slot = byDay[day]?.find(value => value.startTime.slice(11, 16) === time)

                if (!slot) {
                  return (
                    <td key={day} style={{ padding: '4px', borderBottom: '1px solid #f0f0f0', textAlign: 'center' }}>
                      <Box sx={{ height: 36 }} />
                    </td>
                  )
                }
                ...
                return (
                  <td
                    ...
                    style={{
                      ...
                      background: day === new Date().toISOString().slice(0, 10) ? 'rgba(25,118,210,0.03)' : undefined
                    }}
                  >
                    <Stack direction='row' alignItems='center' justifyContent='center' gap={0.5}>
                      ...
                      >
                        <span>
                          ...
                          >
                            <i
                              className={slot.isBlocked ? 'ri-lock-unlock-line' : 'ri-lock-line'}
                              style={{ fontSize: 12 }}
                            />
                          </IconButton>
                        </span>
