...
const MapPointLocationPicker = dynamic(() => import('./MapPointLocationPicker').then(mod => mod.default), {
  ssr: false
})
...
const MapSettingsSection = ({ map, mapPoints, onMapChange, onMapPointsChange }: MapSettingsSectionProps) => {
  ...
  const addPoint = () =>
    onMapPointsChange([...points, createMarker()].map((point, index) => normalizePoint(point, index)))
  ...
  return (
    <Stack gap={3}>
      ...
      <Alert severity='info'>
        Hotels prvo dodaju map pointe ovdje, a onda ih blog sekcije mogu linkati na postojeći point. Adresu možeš
        pretražiti, a zatim po potrebi fino namjestiti pin povlačenjem ili ručnim koordinatama.
      </Alert>
                    ...
                    <Grid item xs={12} md={6}>
                      {(() => {
                        const selectedIconValue =
                          point.icon === 'custom' && point.customPresetId
                            ? `preset:${point.customPresetId}`
                            : point.icon || 'info'
                            ...
                            {MARKER_ICON_OPTIONS.map(option => (
                              <MenuItem key={option.value} value={option.value}>
                                <Stack direction='row' alignItems='center' gap={1}>
                                  {option.iconClass ? (
                                    <i className={option.iconClass} style={{ fontSize: 18 }} />
                                  ) : null}
                      ...
                      <Stack gap={1}>
                        <Stack direction='row' alignItems='center' justifyContent='space-between'>
                          ...
                          <Button
                            ...
                            size='small'
                            onClick={() =>
                              setActivePointMapId(prev =>
                                prev === (point.id || `${index}`) ? null : point.id || `${index}`
                              )
                            }
                          >
