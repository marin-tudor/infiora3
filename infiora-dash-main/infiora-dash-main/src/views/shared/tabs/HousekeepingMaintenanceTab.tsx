...
const schema = yup.object({
  ...
  housekeepingOptions: yup
    .array()
    .of(yup.object({ key: yup.string().default(''), label: yup.string().default(''), icon: yup.string().default('') }))
    .default([]),
  ...
  maintenanceOptions: yup
    .array()
    .of(yup.object({ key: yup.string().default(''), label: yup.string().default('') }))
    .default([])
})
...
const HousekeepingMaintenanceTab: React.FC<Props> = ({ room, group }) => {
  ...
  const hotelId =
    (s?.hotel as any)?.id || (s?.hotel as any)?._id || (typeof s?.hotel === 'string' ? s.hotel : undefined)
  ...
  const {
    ...
    defaultValues: {
      ...
      housekeepingOptions: (s?.housekeeping as any)?.options?.length
        ? (s?.housekeeping as any).options
        : defaultHousekeepingOptions,
      ...
      maintenanceOptions: (s?.maintenance as any)?.options?.length
        ? (s?.maintenance as any).options
    ...
    const payload = {
      housekeeping: {
        ...
        options: (data.housekeepingOptions || [])
          ...
          .map(option => ({
            key: option.key || slugify(option.label || ''),
            label: option.label,
  ...
  return (
    <Stack gap={3} sx={{ maxWidth: 520 }}>
      ...
      <Typography variant='body2' color='text.secondary'>
        {t.subtitle ||
          'Allow guests to submit housekeeping requests or report maintenance issues directly from their room guide.'}
      </Typography>
            ...
            <Stack gap={1}>
              ...
              {housekeepingOptions.fields.map((option, index) => (
                <Stack key={option.id} direction='row' gap={1} alignItems='center'>
                  <Controller
                    name={`housekeepingOptions.${index}.label`}
                    control={control}
                    render={({ field }) => (
                      <TextField
                        {...field}
                        ...
                        onBlur={e => {
                          field.onBlur()
                          setValue(`housekeepingOptions.${index}.key`, slugify(e.target.value))
                        }}
                      />
                    )}
                  />
                  ...
                  />
                  <IconButton color='error' onClick={() => housekeepingOptions.remove(index)}>
              ...
              <Button
                size='small'
                variant='outlined'
                onClick={() => housekeepingOptions.append({ key: '', label: '', icon: '' })}
              >
                Add housekeeping option
              </Button>
            </Stack>
            <Stack gap={1}>
              <Typography variant='body2' color='text.secondary'>
              ...
              <Stack direction='row' alignItems='center' gap={2}>
                ...
                {housekeepingIcon && (
                  <Button
                    ...
                    variant='text'
                    color='secondary'
                    onClick={() => setValue('housekeepingIcon', '')}
                  >
                    Remove icon
                  </Button>
                )}
          ...
          <>
            ...
            <Stack gap={1}>
              ...
              {maintenanceOptions.fields.map((option, index) => (
                <Stack key={option.id} direction='row' gap={1} alignItems='center'>
                  <Controller
                    name={`maintenanceOptions.${index}.label`}
                    control={control}
                    render={({ field }) => (
                      <TextField
                        {...field}
                        ...
                        onBlur={e => {
                          field.onBlur()
                          setValue(`maintenanceOptions.${index}.key`, slugify(e.target.value))
                        }}
                      />
                    )}
                  />
                  <IconButton color='error' onClick={() => maintenanceOptions.remove(index)}>
                    <i className='ri-delete-bin-line' />
                  </IconButton>
                </Stack>
              ))}
              <Button size='small' variant='outlined' onClick={() => maintenanceOptions.append({ key: '', label: '' })}>
                Add maintenance option
              </Button>
            </Stack>
            <Stack gap={1}>
              <Typography variant='body2' color='text.secondary'>
