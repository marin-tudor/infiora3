...
import { getHotelImageUrl, resolveAssetUrl } from '@/utils/imageUrlUtils'
...
const PopupDialog: React.FC<PopupDialogProps> = ({ popup }) => {
  const getBackgroundImage = () => {
    ...
    const imgSrc = popup.image instanceof File ? URL.createObjectURL(popup.image) : resolveAssetUrl(popup.image)
...
const getBgImageUrl = (bg: any): string | null => {
  ...
  return resolveAssetUrl(bg.image as string)
}
      ...
      <Stack alignItems='center' gap={5} width={250}>
        <Box
          ...
          sx={{
            ...
            scrollbarWidth: 'none',
            '&::before':
              bgImageUrl && (room.background?.imageOpacity ?? 1) < 1
                ? {
                    content: '""',
                    position: 'absolute',
                    inset: 0,
        ...
        >
          <Stack gap={2} sx={{ position: 'relative', zIndex: 1 }}>
            ...
            {room.survey?.isActive && room.survey?.type === 'button' && !isNullOrEmpty(room.survey?.mainButtonText) && (
              <Stack mx={5}>
                <Button
                  variant={room.button?.variant || 'contained'}
                  fullWidth
            ...
            {features.housekeepingEnabled !== false && room.housekeeping?.isActive && (
              <Stack mx={5}>
                <Button
                  ...
                  sx={getButtonStyles(room)}
                  startIcon={
                    room.housekeeping?.icon ? <IconPickerItem size={20} value={room.housekeeping.icon} /> : undefined
                  }
                >
            ...
            {features.maintenanceEnabled !== false && room.maintenance?.isActive && (
              <Stack mx={5}>
                <Button
                  ...
                  sx={getButtonStyles(room)}
                  startIcon={
                    room.maintenance?.icon ? <IconPickerItem size={20} value={room.maintenance.icon} /> : undefined
                  }
                >
