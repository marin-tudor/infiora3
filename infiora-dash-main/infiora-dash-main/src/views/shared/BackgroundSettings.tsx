  ...
  const previewImage =
    pendingFile instanceof File
      ...
      : typeof background?.image === 'string'
        ? resolveAssetUrl(background.image)
        : undefined
            ...
            {background?.type === 'image' && (
              <Stack gap={1}>
                ...
                {background?.image && typeof background.image === 'string' && (
                  <Box
                    component='img'
                    src={resolveAssetUrl(background.image)}
                ...
                {background?.backgroundFit && (
                  <Typography variant='caption' color='text.secondary'>
                    Mode:{' '}
                    {FIT_OPTIONS.find(f => f.value === background.backgroundFit)?.label || background.backgroundFit}
                    {background?.backgroundPosition
                      ? ` · Position: ${POSITION_OPTIONS.find(p => p.value === background.backgroundPosition)?.label || background.backgroundPosition}`
                      : ''}
                  </Typography>
                )}
                <Stack gap={0.5}>
                  <Typography variant='body2'>
                    Overlay darkness: {Math.round((1 - (background?.imageOpacity ?? 1)) * 100)}%
                  </Typography>
                  <Slider
