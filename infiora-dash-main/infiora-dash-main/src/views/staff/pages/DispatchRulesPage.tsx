...
const normalizeTargetGroupId = (group: IDispatchRule['targetGroupId']) =>
...
export default function DispatchRulesPage() {
  ...
  return (
    <Stack gap={3}>
      ...
      <Alert severity='info'>
        Dispatch rules decide which tablet group receives a new order. Lower priority runs first, so put your most
              ...
              {localRules.length === 0 ? (
                ...
                localRules.map(rule => (
                  ...
                  >
                    ...
                    <TableCell>
                      {groupNames[normalizeTargetGroupId(rule.targetGroupId)] ||
                        normalizeTargetGroupId(rule.targetGroupId)}
                    </TableCell>
        ...
        <DialogContent>
          <Stack gap={2} sx={{ pt: 1 }}>
                  ...
                  (selected as string[]).length === 0 ? (
                    ...
                    <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
                      {(selected as string[]).map(value => (
                        <Chip
                          key={value}
                          label={itemsList.find(item => item.id === value)?.name || value}
                          size='small'
                        />
                      ))}
                    </Box>
            ...
            <Stack direction='row' alignItems='center' justifyContent='space-between'>
              ...
              />
              <Switch checked={form.active} onChange={e => setForm(prev => ({ ...prev, active: e.target.checked }))} />
            </Stack>
          </Stack>
        </DialogContent>
        <DialogActions>
