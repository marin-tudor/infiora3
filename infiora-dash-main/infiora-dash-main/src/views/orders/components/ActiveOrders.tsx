...
export default function ActiveOrders({ hotelId, currency }: { hotelId: string; currency?: string }) {
  ...
  const { data: carryData } = useGetOrdersQuery({ hotelId, limit: 50 }, { skip: !isToday || !!statusFilter })
  ...
  const carryOrders: IGuestOrder[] =
    isToday && !statusFilter
      ? (carryData?.results || []).filter(
          (o: IGuestOrder) =>
            new Date((o as any).createdAt) < todayStart && !['Completed', 'Cancelled'].includes(o.status)
        )
      : []

  const allOrders = [...mainOrders, ...carryOrders.filter(o => !mainOrders.find(m => m.id === o.id))].sort(
        ...
        <Stack gap={1.5}>
          {carryOrders.length > 0 && (
            <Typography variant='caption' color='warning.main' fontWeight={600}>
              {carryOrders.length} {t.unfinishedCarryPrefix || 'unfinished order'}
              {carryOrders.length > 1 ? t.unfinishedCarryPlural || 's' : ''}{' '}
              {t.unfinishedCarrySuffix || 'carried over from previous days'}
            </Typography>
          )}
          {allOrders.map(order => (
