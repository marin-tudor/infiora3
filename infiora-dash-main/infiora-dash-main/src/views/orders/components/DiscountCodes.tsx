    ...
    <Dialog open={open} onClose={onClose} maxWidth='sm' fullWidth>
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          ...
          <Stack spacing={0.5}>
            <Typography variant='caption' color='text.secondary' fontWeight={600}>Description (internal)</Typography>
            <TextField value={form.description} fullWidth onChange={e => set('description', e.target.value)} placeholder='Internal description' />
          </Stack>
          <Stack direction='row' spacing={2}>
            <Stack spacing={0.5} flex={1}>
              <Typography variant='caption' color='text.secondary' fontWeight={600}>Discount type</Typography>
            ...
            <Stack spacing={0.5} flex={1}>
              <Typography variant='caption' color='text.secondary' fontWeight={600}>
                {form.discountType === 'percentage' ? 'Value (%)' : 'Value (amount)'}
              </Typography>
              <TextField
          ...
          <Stack spacing={0.5}>
            <Typography variant='caption' color='text.secondary' fontWeight={600}>Applies to categories</Typography>
            <FormControl fullWidth>
              <Select
                multiple value={form.applicableCategories} displayEmpty
          ...
          <Stack direction='row' spacing={2}>
            <Stack spacing={0.5} flex={1}>
              <Typography variant='caption' color='text.secondary' fontWeight={600}>Valid from</Typography>
              <TextField type='date' value={form.validFrom} fullWidth onChange={e => set('validFrom', e.target.value)} />
            </Stack>
            <Stack spacing={0.5} flex={1}>
              <Typography variant='caption' color='text.secondary' fontWeight={600}>Valid to</Typography>
              <TextField type='date' value={form.validTo} fullWidth onChange={e => set('validTo', e.target.value)} />
            </Stack>
          </Stack>
          <Stack direction='row' spacing={2}>
            <Stack spacing={0.5} flex={1}>
              <Typography variant='caption' color='text.secondary' fontWeight={600}>Max uses</Typography>
              <TextField type='number' value={form.maxUses} fullWidth onChange={e => set('maxUses', e.target.value)} placeholder='Blank = unlimited' />
            </Stack>
            <Stack spacing={0.5} flex={1}>
              <Typography variant='caption' color='text.secondary' fontWeight={600}>Min order amount</Typography>
              <TextField type='number' value={form.minOrderAmount} fullWidth onChange={e => set('minOrderAmount', e.target.value)} placeholder='Optional' />
            </Stack>
          </Stack>
...
export default function DiscountCodes({ hotelId }: Props) {
  ...
  return (
    <Box>
      ...
      <CodeDialog key={editing?.id ?? 'new'} open={dialogOpen} onClose={() => setDialogOpen(false)} hotelId={hotelId} editing={editing} />
    </Box>
  )
}
