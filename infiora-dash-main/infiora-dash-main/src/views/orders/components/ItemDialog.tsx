...
export default function ItemDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      setBookingConfig(
        item?.bookingConfig ? { ...DEFAULT_BOOKING_CONFIG, ...item.bookingConfig } : DEFAULT_BOOKING_CONFIG
      )
      setAddonName('')
      setAddonPrice('')
      setAddonDescription('')
  ...
  const handleWeekdayTime = (day: WeekDay, field: 'from' | 'to', value: string) => {
    ...
    setBC({
      weeklySchedule: {
        ...bc.weeklySchedule,
        [day]:
          existing.length > 0 ? [{ ...existing[0], [field]: value }] : [{ from: '08:00', to: '22:00', [field]: value }]
      }
    })
  }
    ...
    <Dialog open={open} onClose={onClose} maxWidth='sm' fullWidth>
      ...
      <DialogContent>
        <Stack gap={2} mt={1}>
          ...
          {/* Type toggle */}
          <ToggleButtonGroup value={itemType} exclusive onChange={(_, v) => v && setItemType(v)} size='small' fullWidth>
          ...
          <Stack gap={1}>
            <Typography variant='caption' color='text.secondary' fontWeight={600}>
              {t.imageIcon || 'Image / Icon'}
            </Typography>
            <ButtonGroup size='small' fullWidth>
              {IMAGE_TYPES.map(type => (
                <Button
                  key={type}
                  variant={imageType === type ? 'contained' : 'outlined'}
                ...
                <AccordionDetails>
                  <Stack gap={1.5}>
                    <Stack gap={0.5}>
                      ...
                      <Select
                        value={bc.bookableCategory}
                        onChange={e => setBC({ bookableCategory: e.target.value })}
                        size='small'
                        fullWidth
                      >
                  ...
                  <Stack gap={1.5}>
                    <Stack direction='row' gap={2}>
                      ...
                      <Stack gap={0.5} flex={1}>
                        ...
                        <Select
                          value={bc.startInterval}
                          onChange={e => setBC({ startInterval: Number(e.target.value) })}
                          size='small'
                          fullWidth
                        >
                    ...
                    <FormControlLabel
                      ...
                      label={
                        <Typography variant='caption' fontWeight={600}>
                          Simple availability (same every day)
                        </Typography>
                      }
                    />
                          ...
                          return (
                            <Stack key={day} direction='row' gap={1} alignItems='center'>
                              <Checkbox
                                ...
                                sx={{ p: 0.5 }}
                              />
                              <Typography variant='caption' sx={{ width: 32, textTransform: 'capitalize' }}>
                                {day}
                              </Typography>
                              <TextField
                                value={window0.from}
                ...
                <AccordionDetails>
                  <Stack gap={1.5}>
                    ...
                    <Stack gap={0.5}>
                      ...
                      <RadioGroup
                        row
                        value={bc.confirmationType}
                        onChange={e => setBC({ confirmationType: e.target.value })}
                      >
