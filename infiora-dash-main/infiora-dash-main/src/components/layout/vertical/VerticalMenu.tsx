...
import { useTrackCheckinUsageMutation } from '@/redux/api/ordersApi'
...
const VerticalMenu = ({ dictionary, scrollMenu }: Props) => {
  ...
  const [trackCheckinUsage] = useTrackCheckinUsageMutation()
  ...
  const { data: bookingPendingData } = useGetBookingsQuery(
    { hotelId, status: 'pending', limit: 1 },
    { skip: !hotelId || hotelFeatures?.bookableServicesEnabled === false }
  )
  ...
  const handleGuestCheckinClick = () => {
    window.open('https://dash.eprijava.hr', '_blank', 'noopener,noreferrer')

    if (!hotelId) return

    void trackCheckinUsage({ hotelId }).unwrap().catch(() => {})
  }
      ...
      >
        ...
        {hotelFeatures?.bookableServicesEnabled && (
          <MenuItem
            ...
            onClick={() => handleMenuClick(`/${locale}/bookings`)}
          >
            <MenuLabelWithCount label='Bookings' count={bookingPendingData?.totalResults} />
          </MenuItem>
        )}
        ...
        <MenuItem
          ...
          onClick={handleGuestCheckinClick}
        >
          Guests Check In
        </MenuItem>
