...
export default function OrderNotificationBell() {
  ...
  return (
    <Tooltip
      title={
        pendingCount > 0 ? `${pendingCount} unfinished order${pendingCount > 1 ? 's' : ''}` : 'No unfinished orders'
      }
    >
      <IconButton
        onClick={() => router.push(getLocalizedUrl('/orders', locale as Locale))}
        color={pendingCount > 0 ? 'warning' : 'default'}
      >
        <Badge badgeContent={pendingCount || undefined} color='warning'>
          <i
            className={pendingCount > 0 ? 'ri-notification-3-fill' : 'ri-notification-3-line'}
            style={{ fontSize: 22 }}
          />
        </Badge>
      </IconButton>
