...
import { Button, Menu, MenuItem, Popover, Stack } from '@mui/material'
import {
  format,
  startOfToday,
  subDays,
...
const RangePicker = ({ range, setRange }: RangePickerProps) => {
  ...
  const [selectionRange, setSelectionRange] = useState({
    startDate: range.startDate ? new Date(range.startDate) : subDays(startOfToday(), 6),
    endDate: range.endDate ? new Date(range.endDate) : endOfToday(),
    key: 'selection'
  })

  const [selectedOption, setSelectedOption] = useState<string>(range?.startDate ? 'Custom' : 'Today')
  ...
  return (
    <Stack spacing={2}>
      ...
      <Button startIcon={<CalendarToday />} endIcon={<ExpandMore />} variant='outlined' onClick={handleButtonClick}>
        {selectedOption === 'Custom'
          ? `${format(selectionRange.startDate, 'MMM d, yyyy')} - ${format(selectionRange.endDate, 'MMM d, yyyy')}`
          : selectedOption}
      </Button>
      ...
      >
        {['Today', 'Yesterday', 'This Week', 'Last 7 Days', 'This Month', 'Last 30 Days', 'Custom'].map(option => (
          <MenuItem key={option} onClick={() => handleSelectChange(option)}>
            {option}
          </MenuItem>
        ))}
      </Menu>
      ...
      >
        <Stack sx={{ padding: 2 }}>
          <DateRangePicker locale={enGB} ranges={[selectionRange]} onChange={handleCustomSelect} />
        </Stack>
      </Popover>
    </Stack>
  )
