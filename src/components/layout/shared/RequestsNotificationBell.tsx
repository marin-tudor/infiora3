...
export default function RequestsNotificationBell() {
  ...
  if (!hotelId || (hotelFeatures?.housekeepingEnabled === false && hotelFeatures?.maintenanceEnabled === false)) return null
      ...
      <Menu anchorEl={anchorEl} open={!!anchorEl} onClose={() => setAnchorEl(null)}>
        {housekeepingEnabled && (
          <MenuItem onClick={() => goTo('/housekeeping')}>
            <ListItemText primary={t.housekeeping || dictionary.housekeeping || 'Housekeeping'} secondary={`${housekeepingCount} ${t.unresolved || 'unresolved'}`} />
            <Chip label={housekeepingCount} color='error' size='small' sx={{ ml: 3 }} />
          </MenuItem>
        )}
        {maintenanceEnabled && (
          <MenuItem onClick={() => goTo('/maintenance')}>
            <ListItemText primary={t.maintenance || dictionary.maintenance || 'Maintenance'} secondary={`${maintenanceCount} ${t.unresolved || 'unresolved'}`} />
