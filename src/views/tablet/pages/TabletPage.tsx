...
export default function TabletPage() {
  ...
  const loadActiveOrders = useCallback(async (token: string) => {
    if (!hotelId) return []
  ...
  const loadPastOrders = useCallback(async (token: string) => {
    if (!hotelId) return []
      ...
      setOrders(prev =>
        prev
          .map(order => {
            ...
            if (action === 'cancel') {
              setPastOrders(current =>
                mergeOrders(current, [{ ...order, status: 'Cancelled' }])
              )
            ...
            if (action === 'advance') {
              ...
              if (nextStatus === 'Completed') {
                setPastOrders(current =>
                  mergeOrders(current, [{ ...order, status: 'Completed' }])
                )
              }
    ...
    return (
      <div style={styles.center}>
        <div style={styles.panel}>
          ...
          <p style={styles.panelText}>Store a valid device token in localStorage under `deviceToken` and reload this page.</p>
        </div>
      </div>
    )
  }
  ...
  if (!session) {
    return (
      <div style={styles.center}>
        <div style={{ ...styles.panel, ...(pendingOrdersCount > 0 ? (alertFlashOn ? styles.alertPanelStrong : styles.alertPanelSoft) : null) }}>
    ...
    <div style={styles.shell}>
      ...
      {orders.length === 0 ? (
        <div style={styles.emptyState}>
          ...
          <p style={styles.panelText}>New orders for this group will appear here automatically, and accepted orders will stay visible until they are completed.</p>
        </div>
      ) : (
                  ...
                  {section.orders.map(order => {
                    ...
                    return (
                      <article key={order.orderId} style={styles.card}>
                        ...
                        {order.scheduledFor && <p style={styles.scheduled}>Scheduled for {new Date(order.scheduledFor).toLocaleString()}</p>}

                        <div style={styles.actions}>
                          {order.status === 'Awaiting confirmation' && session.permissions.includes('orders:accept') && (
                            <button
                              type='button'
                  ...
                  <div style={styles.grid}>
                    {section.orders.map(order => (
                      <article key={`${order.orderId}-${order.status}`} style={{ ...styles.card, ...styles.pastCard }}>
                        ...
                        <div style={styles.metaRow}>
                          <span>{formatCurrency(order.total || 0)}</span>
                          <span style={{ ...styles.statusPill, ...(order.status === 'Completed' ? styles.completedPill : styles.cancelledPill) }}>
                            {order.status}
                          </span>
                        </div>
        ...
        <div style={styles.dialogOverlay}>
          <div style={styles.dialogCard}>
            ...
            <div style={styles.dialogSection}>
              ...
              <div style={styles.dialogSummary}>
                <div style={styles.dialogMeta}>
                  Infiora room {acceptDialogOrder.roomNumber || 'N/A'} · Guest room {acceptDialogOrder.roomNumber || 'N/A'}
                </div>
            ...
            <div style={styles.dialogActions}>
              ...
              <button
                type='button'
                onClick={() => void postOrderAction(acceptDialogOrder._id || acceptDialogOrder.id || acceptDialogOrder.orderId, 'accept', {
                  acceptedEta: acceptEtaValue,
                  message: acceptMessage.trim() || undefined
                })}
                disabled={actionOrderId === (acceptDialogOrder._id || acceptDialogOrder.id || acceptDialogOrder.orderId)}
