...
export default function AnalyticsPage() {
  ...
  const totalRevenue = revenueData.reduce((sum: number, entry: { revenue: number }) => sum + entry.revenue, 0) + Number(data?.bookings?.revenue ?? 0)
  const avgRating =
    ratingData.length > 0
      ...
      {data && (
        <>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6} md={2}>
              <MetricCard title='Avg rating' value={avgRating ? `${avgRating.toFixed(1)} star` : 'N/A'} sub='Daily average' />
            </Grid>
          </Grid>
