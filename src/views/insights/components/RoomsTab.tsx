...
export default function RoomsTab({
  insights, hotelId, startDate, endDate
}: {
  insights: IInsights
  hotelId: string
  startDate: string
...
return withRating.reduce((s: number, f: IFeedbackSubmission) => s + (f.rating ?? 0), 0) / withRating.length
  ...
  // Views trend
  const trendSeries = [
    { name: dictionary.views, data: getSeriesData(insights.overTime?.views) },
  ]
  ...
  return (
    <Stack gap={3}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={6} sm={4} md={2}>
          <KpiCard label={t.totalViews || 'Total Views'} value={km.views ?? 0} color='primary.main'
            sub={ch?.views != null ? `${ch.views >= 0 ? '+' : ''}${ch.views.toFixed(1)}% vs prev` : undefined} />
        </Grid>
        <Grid item xs={6} sm={4} md={2}>
          <KpiCard label={t.totalTaps || 'Total Taps'} value={km.taps ?? 0}
            sub={ch?.taps != null ? `${ch.taps >= 0 ? '+' : ''}${ch.taps.toFixed(1)}% vs prev` : undefined} />
        </Grid>
        <Grid item xs={6} sm={4} md={2}>
          <KpiCard label={t.avgTimeSpent || 'Avg Time Spent'} value={formatTime(km.timeSpent || 0) || '0s'} color='info.main' />
        </Grid>
        <Grid item xs={6} sm={4} md={2}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6} md={4}>
          <Card variant='outlined'>
            <CardHeader title={dictionary.pages.insights?.topRooms?.title || 'Top Rooms by Views'} titleTypographyProps={{ variant: 'body1', fontWeight: 600 }} />
            <CardContent sx={{ pt: 0 }}>
              {top10.length > 0 ? (
                <AppReactApexCharts type='bar' width='100%' height={240} options={barOptions} series={barSeries} />
              ) : (
                <Typography variant='body2' color='text.secondary' textAlign='center' py={6}>{t.noData || 'No data'}</Typography>
              )}
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <Card variant='outlined' sx={{ height: '100%' }}>
            <CardHeader title={t.bounceRateSplit || 'Bounce Rate Split'} titleTypographyProps={{ variant: 'body1', fontWeight: 600 }} />
            <CardContent sx={{ pt: 0 }}>
              {rooms.length > 0 ? (
                <AppReactApexCharts
                  type='donut' width='100%' height={240}
                  options={bounceOptions} series={[buckets.low, buckets.mid, buckets.high]}
                />
              ) : (
                <Typography variant='body2' color='text.secondary' textAlign='center' py={6}>{t.noData || 'No data'}</Typography>
              )}
      ...
      {hasFeedback && (
        <Grid container spacing={2}>
          <Grid item xs={12} md={6}>
            <Card variant='outlined'>
              <CardHeader title={t.guestRatingDistribution || 'Guest Rating Distribution'} titleTypographyProps={{ variant: 'h6' }}
                subheader={`${fbData?.results?.length || 0} ${t.totalReviews || 'total reviews'} · ${t.avgRating || 'Avg'} ${globalAvgRating?.toFixed(1) ?? 'N/A'} ★`} />
              <CardContent sx={{ pt: 0 }}>
                <AppReactApexCharts type='bar' width='100%' height={200} options={ratingBarOptions} series={ratingBarSeries} />
              </CardContent>
            </Card>
          </Grid>
          <Grid item xs={12} md={6}>
            <Card variant='outlined' sx={{ height: '100%' }}>
              <CardHeader title={t.roomsByFeedbackRating || 'Rooms by Feedback Rating'} titleTypographyProps={{ variant: 'h6' }}
...
return { room, avg, count: fb.count }
      ...
      {rooms.length > 0 && (
        <Card variant='outlined'>
          <CardHeader title={t.performanceOverview || 'Performance Overview'} titleTypographyProps={{ variant: 'h6' }}
            subheader={t.classifiedByPerformance || 'Classified by bounce rate, time spent and guest ratings'} />
          <CardContent>
            <Grid container spacing={2}>
              <Grid item xs={12} md={4}>
                <Box sx={{ p: 2, borderRadius: 2, border: 2, borderColor: 'success.light', bgcolor: 'success.lighter' }}>
                  <Stack direction='row' gap={1} alignItems='center' mb={1}>
                    <Box sx={{ width: 30, height: 30, borderRadius: '50%', bgcolor: 'success.main', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <i className='ri-award-line' style={{ color: '#fff', fontSize: 15 }} />
                    </Box>
                    <Typography fontWeight={700} color='success.main'>{t.topPerformers || 'Top Performers'}</Typography>
                    <Chip size='small' label={topRooms.length} color='success' sx={{ ml: 'auto' }} />
                  </Stack>
                  <Typography variant='caption' color='text.secondary' component='div' mb={1}>{t.topPerformersHint || 'Bounce < 30% and time > 2min'}</Typography>
                  <Stack direction='row' flexWrap='wrap' gap={0.5}>
                    {topRooms.length > 0
                      ? topRooms.map(r => <Chip key={r.id} size='small' label={`${dictionary.room} ${r.number}`} color='success' variant='outlined' />)
                      : <Typography variant='body2' color='text.secondary'>{t.noRoomsQualifyYet || 'No rooms qualify yet'}</Typography>}
                  </Stack>
                </Box>
              </Grid>
              <Grid item xs={12} md={4}>
                <Box sx={{ p: 2, borderRadius: 2, border: 2, borderColor: 'info.light', bgcolor: 'info.lighter' }}>
                  <Stack direction='row' gap={1} alignItems='center' mb={1}>
                    <Box sx={{ width: 30, height: 30, borderRadius: '50%', bgcolor: 'info.main', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <i className='ri-bar-chart-line' style={{ color: '#fff', fontSize: 15 }} />
                    </Box>
                    <Typography fontWeight={700} color='info.main'>{t.average || 'Average'}</Typography>
                    <Chip size='small' label={avgRooms.length} color='info' sx={{ ml: 'auto' }} />
                  </Stack>
                  <Typography variant='caption' color='text.secondary' component='div' mb={1}>{t.moderateEngagement || 'Moderate engagement'}</Typography>
                  <Stack direction='row' flexWrap='wrap' gap={0.5}>
                    {avgRooms.length > 0
                      ? avgRooms.map(r => <Chip key={r.id} size='small' label={`${dictionary.room} ${r.number}`} color='info' variant='outlined' />)
                      : <Typography variant='body2' color='text.secondary'>{t.noneInThisRange || 'None in this range'}</Typography>}
                  </Stack>
                </Box>
              </Grid>
              <Grid item xs={12} md={4}>
                <Box sx={{ p: 2, borderRadius: 2, border: 2, borderColor: 'error.light', bgcolor: 'error.lighter' }}>
                  <Stack direction='row' gap={1} alignItems='center' mb={1}>
                    <Box sx={{ width: 30, height: 30, borderRadius: '50%', bgcolor: 'error.main', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <i className='ri-alert-line' style={{ color: '#fff', fontSize: 15 }} />
                    </Box>
                    <Typography fontWeight={700} color='error.main'>{t.needsAttention || 'Needs Attention'}</Typography>
                    <Chip size='small' label={attentionRooms.length} color='error' sx={{ ml: 'auto' }} />
                  </Stack>
                  <Typography variant='caption' color='text.secondary' component='div' mb={1}>{t.needsAttentionHint || 'High bounce, low time or bad ratings'}</Typography>
                  <Stack direction='row' flexWrap='wrap' gap={0.5}>
                    {attentionRooms.length > 0
                      ? attentionRooms.map(r => <Chip key={r.id} size='small' label={`${dictionary.room} ${r.number}`} color='error' variant='outlined' />)
                      : <Typography variant='body2' color='text.secondary'>{t.allRoomsPerformingWell || 'All rooms performing well'}</Typography>}
                  </Stack>
                </Box>
              </Grid>
            </Grid>
      ...
      {sortedRooms.length > 0 && (
        <Box>
          <Typography variant='h6' mb={2}>{t.roomDetailCards || 'Room Detail Cards'}</Typography>
          <Grid container spacing={2}>
                  ...
                  <Card variant='outlined' sx={{
                    ...
                    <CardContent>
                      <Stack direction='row' justifyContent='space-between' alignItems='flex-start' mb={1.5}>
                        <Box>
                          <Typography variant='h5' fontWeight={700} color='primary.main'>{dictionary.room} {room.number || 'N/A'}</Typography>
                          {room.group && <Chip size='small' label={room.group.title} color='secondary' sx={{ mt: 0.5 }} />}
                        </Box>
                        {avgRating != null ? (
                          <Box sx={{ textAlign: 'right' }}>
                            ...
                            <Typography variant='caption' color='text.secondary'>{fb!.count} {t.reviews || 'reviews'}</Typography>
                          </Box>
                        ) : (
                          <Typography variant='caption' color='text.disabled'>{t.noRatings || 'No ratings'}</Typography>
                        )}
                      </Stack>
                      ...
                      <Stack gap={0.75}>
                        <Stack direction='row' justifyContent='space-between'>
                          <Typography variant='caption' color='text.secondary'>{dictionary.timeSpent}</Typography>
                        ...
                        <Stack direction='row' justifyContent='space-between' mt={0.5}>
                          <Typography variant='caption' color='text.secondary'>{dictionary.bounceRate}</Typography>
                          <Typography variant='caption' fontWeight={600} color={bounceColor(br)}>{br}%</Typography>
                        </Stack>
                        <Box sx={{ height: 5, bgcolor: 'action.hover', borderRadius: 3, overflow: 'hidden' }}>
