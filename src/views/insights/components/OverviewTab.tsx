...
import {
  Card, CardContent, CardHeader, Chip, Grid, Stack,
...
export default function OverviewTab({
  ...
  const trendChartData = useMemo(() => {
    const keys = Array.from(
      new Set([
        ...Object.keys(insights.overTime?.views || {}),
        ...Object.keys(insights.overTime?.taps || {})
      ])
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={6} sm={4} md={3}>
          <KpiCard
            ...
            sub={
              ch?.bounceRate != null
                ? `${ch.bounceRate > 0 ? '+' : ''}${ch.bounceRate.toFixed(1)}% vs prev`
                : undefined
            }
          />
        </Grid>
...
return (
                  <TableRow key={room.id} hover>
                    ...
                    <TableCell>
                      {room.group ? (
                        <Chip size='small' label={room.group.title || ''} color='secondary' />
                      ) : (
                        '—'
                      )}
                    </TableCell>
