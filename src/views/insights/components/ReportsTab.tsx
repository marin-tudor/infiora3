...
export default function ReportsTab({
  ...
  const { data: visitAnalytics } = useGetOrderVisitAnalyticsQuery(
    { hotelId, startDate, endDate },
    { skip: !hotelId }
  )
...
return [
  ...
  const ordersRows = (oa?.overTime || []).map((d: any) => [
    d.date,
    d.orders,
    d.revenue?.toFixed(2),
  ])
  ...
  const reports = [
    {
      ...
      description: t.analyticsOverviewDesc || 'Daily views, taps, bounce rate and time spent across the selected period.',
    ...
    },
    {
      ...
      description: t.roomPerformanceDesc || 'Per-room breakdown with views, bounce rate, time spent, taps, and guest ratings.',
      rows: roomRows.length,
      onDownload: () =>
        downloadCSV(
          `room-performance-${periodLabel}.csv`,
          ['Room #', 'Group', 'Views', 'Bounce Rate %', 'Time Spent (s)', 'Time Spent', 'Taps', 'Avg Rating', 'Review Count'],
          roomRows
        ),
    },
    {
      ...
      description: t.buttonInteractionsDesc || 'All button/link tap counts, source room or group, and percentage share of total taps.',
    ...
    },
    {
      ...
      description: t.ordersReportDesc || 'Daily order counts and revenue. Includes total summary, completion rate and avg order value.',
    ...
    },
    {
      ...
      description: t.guestFeedbackDesc || 'All guest reviews collected in the period: room, star rating, message and optional email.',
      rows: fbRows.length,
      onDownload: () =>
        downloadCSV(
  ...
  return (
    <Stack gap={3}>
      ...
      <Card variant='outlined'>
        <CardHeader title={t.dataSummary || 'Data Summary'} titleTypographyProps={{ variant: 'h6' }}
          subheader={t.availableRecords || 'Available records for the selected period'} />
        <CardContent>
          <Grid container spacing={2}>
            {[
              { label: t.dailyViewSnapshots || 'Daily view snapshots', value: overviewRows.length, icon: 'ri-eye-line', color: '#3b82f6' },
              { label: t.roomsTracked || 'Rooms tracked', value: insights?.rooms?.length ?? 0, icon: 'ri-door-line', color: '#10b981' },
              { label: t.buttonRecords || 'Button records', value: buttonRows.length, icon: 'ri-cursor-line', color: '#f59e0b' },
              { label: t.orderDays || 'Order days', value: ordersRows.length, icon: 'ri-shopping-bag-3-line', color: '#8b5cf6' },
              { label: t.feedbackReviews || 'Feedback reviews', value: fbRows.length, icon: 'ri-star-line', color: '#ef4444' },
            ].map(s => (
              <Grid item xs={6} sm={4} md key={s.label}>
                <Stack direction='row' gap={1.5} alignItems='center'>
                  <Box sx={{ width: 36, height: 36, borderRadius: 1.5, bgcolor: `${s.color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
