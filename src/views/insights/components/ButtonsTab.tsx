...
export default function ButtonsTab({ insights }: { insights: IInsights }) {
  ...
  const avgTapsPerView = insights.keyMetrics.views > 0
    ? (totalTaps / insights.keyMetrics.views).toFixed(1)
    : '0'
...
return buckets
  ...
  const donutLabels = [...donutLinks.map(l => l.title || (t.button || 'Button')), ...(othersTotal > 0 ? [t.others || 'Others'] : [])]
  ...
  const peakOptions: ApexOptions = {
    ...
    xaxis: {
      ...
      labels: {
        rotate: -45, style: { colors: theme.palette.text.secondary, fontSize: '10px' }
      }
    },
    yaxis: { labels: { style: { colors: theme.palette.text.secondary }, formatter: v => `${Math.floor(v)}` } },
    grid: { borderColor: theme.palette.divider },
    tooltip: { theme: theme.palette.mode, x: { formatter: (_, opts) => `${String(opts.dataPointIndex).padStart(2, '0')}:00` } },
      ...
      <Grid container spacing={2}>
        <Grid item xs={6} md={3}>
          <Card variant='outlined'>
            <CardContent>
              <Typography variant='caption' color='text.secondary' textTransform='uppercase' letterSpacing={0.5}>{t.totalTaps || 'Total Taps'}</Typography>
              <Typography variant='h4' fontWeight={700} color='primary.main' mt={0.5}>{totalTaps}</Typography>
              <Typography variant='caption' color='text.secondary'>{t.allButtonsCombined || 'All buttons combined'}</Typography>
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={6} md={3}>
          <Card variant='outlined'>
            <CardContent>
              <Typography variant='caption' color='text.secondary' textTransform='uppercase' letterSpacing={0.5}>{t.activeButtons || 'Active Buttons'}</Typography>
              <Typography variant='h4' fontWeight={700} mt={0.5}>{activeButtons}</Typography>
              <Typography variant='caption' color='text.secondary'>{t.of || 'of'} {links.length} {t.total || 'total'}</Typography>
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={6} md={3}>
          <Card variant='outlined'>
            <CardContent>
              <Typography variant='caption' color='text.secondary' textTransform='uppercase' letterSpacing={0.5}>{t.mostTappedButton || 'Most Tapped Button'}</Typography>
        ...
        <Grid item xs={6} md={3}>
          <Card variant='outlined'>
            <CardContent>
              <Typography variant='caption' color='text.secondary' textTransform='uppercase' letterSpacing={0.5}>{t.tapsPerView || 'Taps per View'}</Typography>
              <Typography variant='h4' fontWeight={700} color='warning.main' mt={0.5}>{avgTapsPerView}</Typography>
              <Typography variant='caption' color='text.secondary'>{t.avgPerPageVisit || 'Avg per page visit'}</Typography>
            </CardContent>
          </Card>
        </Grid>
      </Grid>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6} md={4}>
          <Card variant='outlined' sx={{ height: '100%' }}>
            <CardHeader title={t.tapDistribution || 'Tap Distribution'} titleTypographyProps={{ variant: 'body1', fontWeight: 600 }} />
            <CardContent sx={{ pt: 0 }}>
              {donutSeries.some(v => v > 0) ? (
                <AppReactApexCharts type='donut' width='100%' height={240} options={donutOptions} series={donutSeries} />
              ) : (
                <Typography variant='body2' color='text.secondary' textAlign='center' py={6}>{t.noTapData || 'No tap data'}</Typography>
              )}
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <Card variant='outlined' sx={{ height: '100%' }}>
            <CardHeader title={t.peakHours || 'Peak Hours'} titleTypographyProps={{ variant: 'body1', fontWeight: 600 }} />
            <CardContent sx={{ pt: 0 }}>
              {hourBuckets.some(v => v > 0) ? (
                <AppReactApexCharts type='bar' width='100%' height={240} options={peakOptions} series={peakSeries} />
              ) : (
                <Typography variant='body2' color='text.secondary' textAlign='center' py={6}>{t.noActivityData || 'No activity data'}</Typography>
              )}
            </CardContent>
          </Card>
        </Grid>
      ...
      {(topButtons.length > 0 || topGroups.length > 0) && (
        <Grid container spacing={2}>
          <Grid item xs={12} lg={6}>
            <Card variant='outlined' sx={{ height: '100%' }}>
              <CardHeader title={dictionary.pages.insights?.topButtons?.title || 'Top 3 Buttons'} titleTypographyProps={{ variant: 'h6' }} />
              <CardContent>
                {topButtons.length > 0 ? (
                  <Grid container spacing={2}>
...
return (
                        <Grid item xs={12} key={link.id}>
                          <Box sx={{
                            ...
                            <Box sx={{ flex: 1, minWidth: 0 }}>
                              <Typography variant='body2' fontWeight={600} noWrap>{link.title || (t.untitled || 'Untitled')}</Typography>
                              ...
                              <Box sx={{ mt: 0.5, height: 4, bgcolor: 'action.hover', borderRadius: 2, overflow: 'hidden' }}>
                                <Box sx={{ height: '100%', width: `${(link.taps || 0) / maxButtonTaps * 100}%`, bgcolor: color, borderRadius: 2 }} />
                              </Box>
                            </Box>
                  ...
                  <Typography variant='body2' color='text.secondary' textAlign='center' py={6}>{t.noButtonClickData || 'No button click data'}</Typography>
                )}
              </CardContent>
            </Card>
          </Grid>
          <Grid item xs={12} lg={6}>
            <Card variant='outlined' sx={{ height: '100%' }}>
              <CardHeader title={dictionary.pages.insights?.topGroupButtons?.title || 'Top 3 Group Buttons'} titleTypographyProps={{ variant: 'h6' }} />
              <CardContent>
                {topGroups.length > 0 ? (
                  <Grid container spacing={2}>
...
return (
                        <Grid item xs={12} key={link.id}>
                          <Box sx={{
                            ...
                            <Box sx={{ flex: 1, minWidth: 0 }}>
                              <Typography variant='body2' fontWeight={600} noWrap>{link.title || (t.untitledGroup || 'Untitled Group')}</Typography>
                              <Typography variant='caption' color='text.secondary'>{t.measuredByGroupVisits || 'Measured by group visits'}</Typography>
                              <Box sx={{ mt: 0.5, height: 4, bgcolor: 'action.hover', borderRadius: 2, overflow: 'hidden' }}>
                                <Box sx={{ height: '100%', width: `${(link.taps || 0) / maxGroupVisits * 100}%`, bgcolor: color, borderRadius: 2 }} />
                              </Box>
                            </Box>
                  ...
                  <Typography variant='body2' color='text.secondary' textAlign='center' py={6}>{t.noGroupVisitData || 'No group visit data'}</Typography>
                )}
              </CardContent>
            </Card>
          </Grid>
