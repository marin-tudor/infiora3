...
const STATUS_CONFIG: Record<OrderStatus, { label: string; color: 'warning' | 'info' | 'primary' | 'success' | 'error' | 'default' }> = {
...
export default function OrderCard({ order, currency = '€', escalated = false }: { order: IGuestOrder; currency?: string; escalated?: boolean }) {
    ...
    <>
        ...
        <CardContent>
          <Stack direction='row' alignItems='center' justifyContent='space-between' mb={1.5}>
            <Stack direction='row' alignItems='center' gap={1}>
              ...
              <Chip label={`${t.guest || 'Guest'} ${order.guestRoomNumber || (t.notProvided || 'Not provided')}`} size='small' color='secondary' variant='tonal' />
              ...
              {escalated && (
                <Chip label='Escalated' size='small' color='error' variant='filled' />
              )}
            </Stack>
            <Stack direction='row' alignItems='center' gap={1}>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} gap={2} alignItems='flex-start'>
            <Box flex={1}>
              ...
              <Stack direction='row' gap={1.5} flexWrap='wrap' mt={1}>
                ...
                <Typography variant='caption' color='text.secondary'>
                  {(t.guestEnteredRoom || 'Guest entered room')}: <strong>{order.guestRoomNumber || (t.notProvided || 'Not provided')}</strong>
                </Typography>
              </Stack>
              {order.guestEmail && (
            ...
            {(status === 'Awaiting confirmation' || status === 'Processing' || status === 'On the way') && (
              <Stack gap={1} alignItems='flex-end' minWidth={160}>
                {status === 'Awaiting confirmation' ? (
                  <Button variant='contained' size='small' onClick={handleOpenAccept} disabled={loading} fullWidth startIcon={<span>🍴</span>}>
                    {t.acceptOrder || 'Accept Order'}
                  </Button>
                ) : (
                  <Button variant='outlined' color='success' size='small' onClick={handleAdvance} disabled={loading} fullWidth>
                    {NEXT_STATUS[status]}
                  </Button>
                )}
                <Button variant='outlined' color='error' size='small' onClick={handleCancel} disabled={loading} fullWidth>
                  {dictionary.cancel}
                </Button>
              </Stack>
            )}
      ...
      <Dialog open={acceptOpen} onClose={() => setAcceptOpen(false)} maxWidth='sm' fullWidth>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography variant='caption' color='text.secondary' ml='auto'>#{order.orderId}</Typography>
        </DialogTitle>
        <DialogContent>
          <Stack gap={2.5} mt={0.5}>
            ...
            <Box>
              ...
              <Box sx={{ mt: 1, p: 1.5, bgcolor: 'action.hover', borderRadius: 1 }}>
                ...
                <Chip label={roomMatch.label} size='small' color={roomMatch.color as any} variant='outlined' sx={{ mb: 1 }} />
                <Stack gap={0.5}>
                  {order.items?.map((item, i) => (
                    <Stack key={i} direction='row' justifyContent='space-between'>
                      <Typography variant='body2'>{item.qty}× {item.name}</Typography>
                      <Typography variant='body2' color='text.secondary'>{(item.price * item.qty).toFixed(2)} {currency}</Typography>
                    </Stack>
                  ))}
                </Stack>
                <Divider sx={{ my: 1 }} />
                <Stack direction='row' justifyContent='space-between'>
                  <Typography variant='body2' fontWeight={700}>{t.total || 'Total'}:</Typography>
                  <Typography variant='body2' fontWeight={700}>{total.toFixed(2)} {currency}</Typography>
                </Stack>
              </Box>
            </Box>
            ...
            <Box>
              ...
              <ToggleButtonGroup
                value={etaPreset}
                exclusive
                onChange={(_, v) => { if (v !== null) { setEtaPreset(v); setCustomEta('') } }}
                size='small'
                sx={{ mt: 1, flexWrap: 'wrap', gap: 0.5 }}
              >
                {ETA_PRESETS.map(t => (
                  <ToggleButton key={t} value={t} sx={{ minWidth: 64 }}>{t} min</ToggleButton>
                ))}
              </ToggleButtonGroup>
              <Stack direction='row' alignItems='center' gap={1} mt={1.5}>
                <Typography variant='caption' color='text.secondary'>{t.custom || 'Custom'}:</Typography>
                <TextField
                  type='number'
                  size='small'
                  value={customEta}
                  onChange={e => { setCustomEta(e.target.value); setEtaPreset(null) }}
                  placeholder='--'
                  sx={{ width: 80 }}
                  inputProps={{ min: 1 }}
                />
                <Typography variant='caption' color='text.secondary'>{t.minutes || 'minutes'}</Typography>
              </Stack>
            </Box>
        ...
        <DialogActions>
          <Button onClick={() => setAcceptOpen(false)} disabled={loading}>{dictionary.cancel}</Button>
