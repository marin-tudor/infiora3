...
import {
  Stack, Typography, Button, Card, CardContent, Box, IconButton, Chip,
  Switch, Tooltip, Tabs, Tab, TextField, InputAdornment
} from '@mui/material'
...
import {
  ...
  useGetCatalogItemsQuery, useDeleteCatalogItemMutation, useToggleCatalogItemMutation
            ...
            <Stack gap={1}>
              {categoriesList.length === 0 ? (
                <Box textAlign='center' py={6}>
                  ...
                  <Typography color='text.secondary'>{t.noCategoriesYet || 'No categories yet. Add your first one!'}</Typography>
                </Box>
              ) : (() => {
                  ...
                  <Card key={cat.id} variant='outlined' sx={isSubcategory ? { ml: 4, borderStyle: 'dashed' } : {}}>
                    <CardContent sx={{ py: '12px !important' }}>
                      <Stack direction='row' alignItems='center' gap={2}>
                        {isSubcategory && <Typography variant='caption' color='text.secondary' sx={{ flexShrink: 0 }}>↳</Typography>}
                        <Box sx={{ width: 40, height: 40, borderRadius: 1, bgcolor: 'grey.100', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, overflow: 'hidden', flexShrink: 0 }}>
                        ...
                        <Box flex={1}>
                          ...
                          <Typography variant='caption' color='text.secondary'>
                            ...
                            {isSubcategory && <Chip label={t.subcategory || 'subcategory'} size='small' variant='outlined' sx={{ ml: 1, height: 16, fontSize: 10 }} />}
                          </Typography>
                        </Box>
                        <Switch
                          checked={cat.active}
                        ...
                        <Tooltip title={t.filterItemsByCategory || 'Filter items by this category'}>
                          <Button
                            ...
                            onClick={() => { setSelectedCatId(selectedCatId === cat.id ? null : cat.id); setTab(1) }}
                          >
                            {dictionary.items}
                          </Button>
                        </Tooltip>
                        <IconButton
                          size='small'
                          onClick={() => { setEditCategory(cat); setCatDialogOpen(true) }}
                        >
...
return (
                  <Chip
                    ...
                    size='small'
                    avatar={isUrl ? <Box component='img' src={cat.icon} sx={{ width: 20, height: 20, borderRadius: '50%', objectFit: 'cover' }} /> : undefined}
                  />
                )
              })}
            </Stack>
          ...
          {loadingItems ? <Loader /> : (
            <Card variant='outlined'>
              {/* Table header */}
              <Box sx={{ display: 'grid', gridTemplateColumns: '60px 1fr 120px 90px 80px 80px 100px', px: 2, py: 1.5, bgcolor: 'grey.50', borderBottom: '1px solid', borderColor: 'divider' }}>
                {['', dictionary.name, t.category || 'Category', dictionary.price, t.discount || 'Discount', t.available || 'Available', t.actions || 'Actions'].map(h => (
              ...
              {filteredItems.length === 0 ? (
                <Box textAlign='center' py={6}>
                  <Typography fontSize={36} mb={1}>🍽️</Typography>
                  <Typography color='text.secondary'>{t.noItemsFound || 'No items found. Add your first service or product!'}</Typography>
                </Box>
              ) : (
...
return (
                    ...
                    >
                      <Box sx={{ width: 44, height: 44, borderRadius: 1, bgcolor: 'grey.100', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, overflow: 'hidden' }}>
