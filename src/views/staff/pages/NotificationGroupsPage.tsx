...
export default function NotificationGroupsPage() {
  ...
  return (
    <Stack gap={3}>
      ...
      <Alert severity='info'>
        Create one group for each tablet screen. After saving a group, copy its tablet page link and open it on the tablet device.
      </Alert>
                    ...
                    <TableCell>
                      <Stack direction='row' gap={1} flexWrap='wrap'>
                        {(group.emailAddresses || []).length === 0 ? (
                          ...
                          (group.emailAddresses || []).map((email: string) => <Chip key={email} size='small' label={email} />)
                        )}
                      </Stack>
                    </TableCell>
                    <TableCell>
                      <Chip size='small' color={group.sseEnabled ? 'success' : 'default'} label={group.sseEnabled ? 'Enabled' : 'Disabled'} />
                    </TableCell>
                        ...
                        InputProps={{
                          ...
                          endAdornment: (
                            <InputAdornment position='end'>
                              ...
                              <IconButton size='small' component='a' href={getTabletUrl(group.id)} target='_blank' rel='noreferrer'>
