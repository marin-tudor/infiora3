...
const normalizeRoleId = (roleId: IStaffMember['roleId']) =>
  typeof roleId === 'string' ? roleId : roleId?.id || ''

const normalizeGroupIds = (groupIds: IStaffMember['groupIds']) =>
  (groupIds || []).map(group => (typeof group === 'string' ? group : group.id))

const getRoleName = (roleId: IStaffMember['roleId']) => (typeof roleId === 'string' ? roleId : roleId?.name || 'Unknown role')
...
export default function StaffPage() {
  ...
  return (
    <Stack gap={3}>
      ...
      <Alert severity='info'>
        Staff members log into the tablet with a 4-digit PIN. Assign a role for permissions and add one or more notification groups to control which orders they see.
      </Alert>
