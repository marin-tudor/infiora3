...
export default function StaffRolesPage() {
  ...
  return (
    <Stack gap={3}>
      ...
      <Alert severity='info'>
        Roles define what staff can do in tablet mode. Start with order permissions, then enable only the modules each role actually needs.
      </Alert>
            ...
            <Box>
              ...
              <Stack direction='row' flexWrap='wrap'>
                {MODULES.map(moduleName => (
                  <FormControlLabel
                    key={moduleName}
                    control={<Checkbox checked={form.visibleModules.includes(moduleName)} onChange={() => toggleModule(moduleName)} />}
                    label={moduleName}
