...
const FeedbackTab: React.FC<FeedbackTabProps> = ({ room, group, links }) => {
  ...
  const [liveSurvey, setLiveSurvey] = useState<ISurvey>(
    s?.survey ?? { isActive: false, type: 'popup', questions: [] }
  )
  ...
  return (
    <Stack direction={{ xs: 'column', md: 'row' }} gap={5}>
      ...
      <Stack gap={0} flex={1} height={{ md: 550 }} sx={{ position: 'relative', overflowY: 'auto', scrollbarWidth: 'none' }}>
      ...
      <RoomPreview
        room={{ ...s, feedback: watchValues, survey: liveSurvey }}
        links={links}
        showFeedback={true}
      />
    </Stack>
