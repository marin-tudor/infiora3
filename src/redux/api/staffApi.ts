...
export const staffApi = createApi({
  ...
  endpoints: builder => ({
    ...
    createStaffMember: builder.mutation<IStaffMember, { hotelId: string; body: Partial<IStaffMember> & { pin: string } }>({
      query: ({ hotelId, body }) => ({ url: `/v1/hotels/${hotelId}/staff/members`, method: 'POST', body }),
      invalidatesTags: [{ type: 'StaffMembers', id: 'LIST' }]
    }),
    updateStaffMember: builder.mutation<IStaffMember, { hotelId: string; memberId: string; body: Partial<IStaffMember> & { pin?: string } }>({
    ...
    createNotificationGroup: builder.mutation<INotificationGroup, { hotelId: string; body: Partial<INotificationGroup> }>({
      query: ({ hotelId, body }) => ({ url: `/v1/hotels/${hotelId}/dispatch/groups`, method: 'POST', body }),
      invalidatesTags: [{ type: 'NotificationGroups', id: 'LIST' }]
    }),
    updateNotificationGroup: builder.mutation<INotificationGroup, { hotelId: string; groupId: string; body: Partial<INotificationGroup> }>({
    ...
    updateDispatchRule: builder.mutation<IDispatchRule, { hotelId: string; ruleId: string; body: Partial<IDispatchRule> }>({
