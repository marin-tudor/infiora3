...
export const reorderLinks = (room?: IRoom, links?: ILink[]) => {
  ...
  const reordered = (orderedLinks || []).map(id => linkMap.get(id)).filter((link): link is ILink => Boolean(link)) // Type guard to exclude undefined

  // Add remaining links not in orderedLinks
  const remainingLinks = links.filter(link => !orderedLinks?.includes(link.id))
