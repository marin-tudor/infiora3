...
const paginate = <T extends Document, U extends Model<U>>(schema: Schema<T>): void => {
  const normalizeOption = (value?: string | string[]): string => {
  ...
  schema.static('paginate', async function (filter: Record<string, any>, options: IOptions): Promise<QueryResult> {
    ...
    if (options.sortBy) {
      ...
      normalizeOption(options.sortBy).split(',').forEach((sortOption: string) => {
    ...
    if (options.projectBy) {
      ...
      normalizeOption(options.projectBy).split(',').forEach((projectOption) => {
    ...
    if (options.populate) {
      normalizeOption(options.populate).split(',').forEach((populateOption: any) => {
