...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/login', () => {
    ...
    test('should return 403 error if user is inactive', async () => {
      ...
      expect(res.body).toEqual({ code: httpStatus.FORBIDDEN, message: 'Your account is inactive' });
    });

    test('should return 403 error if normal user email is unverified', async () => {
      await insertUsers([{ ...userOne, role: 'user', isEmailVerified: false }]);
      ...
      expect(res.body).toEqual({ code: httpStatus.FORBIDDEN, message: 'Please verify your email before logging in' });
    });
  });
...
describe('Auth middleware', () => {
  ...
  test('should call next with unauthorized error if access token is not found in header', async () => {
    ...
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({ statusCode: httpStatus.UNAUTHORIZED, message: 'Your session has expired, please log in again.' })
    );
  });
