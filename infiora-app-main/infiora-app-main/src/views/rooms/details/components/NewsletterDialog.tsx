...
const NewsletterDialog: React.FC<NewsletterDialogProps> = ({
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!consent) {
      toast.error("Please accept the privacy notice to subscribe.");
      return;
    }
    try {
      setLoading(true);
                ...
                <FormControlLabel
                  ...
                  label={
                    <Typography variant="caption" sx={{ color: "rgba(255,255,255,0.85)" }}>
                      I agree my email may be used to send newsletters.{" "}
                      <NextLink href="/privacy" target="_blank" rel="noopener noreferrer" style={{ color: "white" }}>
                        Privacy Policy
                      </NextLink>
                    </Typography>
                  }
