...
Recommended first action: complete Wave 1 before adding product features. Specifically, create guest-safe public response DTOs, enforce ownership on all authenticated mutations, fix Stripe amount/status verification and idempotency, disable or restrict SSRF-capable fetchers, rotate/remove committed secrets, and add regression tests for those exact flows.
...
| INF-AUDIT-004 | P0 | Security | Guest image proxy/Next images | Arbitrary remote image fetching creates SSRF/open proxy risk | Wave 1 |
| INF-AUDIT-005 | P1 | Security | Environment and seed files | Committed `.env` files, default secrets, uploads, temp payloads, and seed credentials are present | Wave 1 |
| INF-AUDIT-006 | P1 | Privacy | Guest bookings API/UI | Public room booking listing exposes future booking data for a room | Wave 1 |
...
| INF-AUDIT-011 | P1 | Auth | Staff RBAC | Staff role/group IDs are not hotel-scoped and issued staff tokens can retain stale permissions | Wave 1 |
...
**Affected Area:** Remote image fetching

**Problem:**  
The guest image proxy accepts an arbitrary `url` query parameter and fetches any HTTP/HTTPS URL. It has size, timeout, and content-type checks, but it does not enforce the configured `IMAGE_PROXY_ALLOWED_HOSTS`, block private/link-local/metadata IP ranges, or prevent DNS rebinding patterns. The Next.js image configuration also allows `http` and `https` hostnames matching `**`.

**Why It Matters:**  
Server-side image fetching can become an SSRF primitive or an open proxy. Next image optimizers have also had multiple advisories related to broad remote patterns.

**Risk If Ignored:**  
Attackers can probe internal services, cloud metadata endpoints, private network hosts, or abuse the app as an image proxy/cache.

**Suggested Fix:**  
Use a strict allowlist of storage/CDN hosts, enforce HTTPS except local development, resolve and reject private IP ranges, disable redirects or revalidate redirect targets, and remove wildcard `remotePatterns`. If possible, serve only signed/uploaded assets from known storage domains.
...
**Notes:**  
The environment parser already references `IMAGE_PROXY_ALLOWED_HOSTS`; the route should actually enforce it.
...
**Affected Area:** Secrets, local data, and repository hygiene

**Problem:**  
Committed environment files and scripts contain default secrets, local connection strings, placeholder credentials, seed login details, sample reservation codes, temp payloads, and uploaded files. Even placeholder values are dangerous when they become the path of least resistance in development and staging.

**Why It Matters:**  
Credentials and guest-uploaded artifacts should not live in source control. Default secrets are often accidentally reused in deployed environments.

**Risk If Ignored:**  
Session/token compromise in misconfigured environments, leaked guest/hotel data, accidental credential reuse, and noisy audits/compliance failures.

**Suggested Fix:**  
Remove committed `.env` files and local uploaded/temp artifacts from version control, rotate any secrets that may have been used, keep only `.env.example` files with safe placeholders, and add a startup check that rejects default secrets outside local development. Keep seed credentials in documentation generated for local setup only, not committed operational files.
...
**Notes:**  
The root `.gitignore` already ignores many of these patterns, which means the worktree likely contains previously committed or manually added artifacts.
...
**Problem:**  
`GET /v1/bookings/hotels/:hotelId/rooms/:roomId/bookings` is public and returns future active bookings for a room. The guest app displays these as "My Bookings", but no guest token, email proof, reservation code, or booking reference is required.

**Why It Matters:**  
Room-level booking data can reveal guest behavior, booking times, party size, service names, payment method, status, and booking references.

**Risk If Ignored:**  
Privacy breach between guests sharing a room ID, leaked operational data, and possible booking cancellation/social engineering attacks using exposed references.

**Suggested Fix:**  
Replace public listing with token-scoped "my bookings" access. Require a booking status token, email magic link, booking reference plus email, or authenticated staff/admin context. Return only guest-safe fields.
...
**Notes:**  
The product needs to decide whether a room-shared booking board is intentional. If intentional, it must be labeled and minimized accordingly.
...
**Category:** Auth  
**Affected File(s):**
- `infiora-backend-main/infiora-backend-main/src/modules/staff/staff.validation.ts`
- `infiora-backend-main/infiora-backend-main/src/modules/staff/staff.service.ts`
- `infiora-backend-main/infiora-backend-main/src/modules/staff/staffAuth.ts`
- `infiora-backend-main/infiora-backend-main/src/modules/staff/staff.model.ts`
- `infiora-backend-main/infiora-backend-main/src/routes/v1/staff.route.ts`

**Affected Area:** Staff RBAC and token validity

**Problem:**  
Staff member creation and updates accept `roleId` and `groupIds` without verifying that those IDs belong to the same hotel or are allowed templates. Staff JWT verification trusts embedded permissions and does not check whether the staff member is still active or whether role permissions have changed since token issuance.

**Why It Matters:**  
Staff permissions are tenant-sensitive and operationally sensitive.

**Risk If Ignored:**  
Privilege escalation across hotels, stale access after deactivation, and staff continuing to perform actions after permissions are revoked.

**Suggested Fix:**  
Validate role/group ownership on create and update. Include token version or permission version in staff tokens and verify it against the current staff member/role on protected routes. Reject inactive staff at request time. Add tests for cross-hotel role assignment and stale token revocation.
...
**Notes:**  
This is especially important before expanding staff-facing operations.
...
**Notes:**  
Depends on INF-AUDIT-006 and INF-AUDIT-013.
...
- INF-AUDIT-011 shows staff RBAC can assign cross-hotel roles/groups and use stale token permissions.

Exposed secrets and unsafe configuration:
- INF-AUDIT-005 covers committed `.env` files, placeholder/default secrets, seed credentials, uploaded local files, and temp payloads.
...
- INF-AUDIT-006 covers public booking listing privacy exposure.
...
- INF-AUDIT-004 covers the guest app image proxy and broad Next.js remote image patterns.
...
- Guest "My Bookings" appears to show all room bookings instead of token-scoped personal bookings. See INF-AUDIT-006.
...
- Staff permissions need current server-side validation, not only embedded token claims. See INF-AUDIT-011.
...
- Environment parsing exists in places but is not used consistently by all app code. See INF-AUDIT-030 and INF-AUDIT-004.
...
- Image handling is both a security and performance issue due to broad remote fetch behavior. See INF-AUDIT-004.
...
- Remote image optimization should be restricted to known hosts. See INF-AUDIT-004.
...
- Staff role/group IDs need hotel-scoped validation. See INF-AUDIT-011.
...
- Tenant isolation must be fixed first through ownership checks and guest-safe DTOs. See INF-AUDIT-001, INF-AUDIT-002, INF-AUDIT-011.
...
- URL safety validators for image proxy and iCal sync. See INF-AUDIT-004 and INF-AUDIT-012.
...
- Public booking privacy behavior. See INF-AUDIT-006.
- Maintenance/housekeeping validation and status updates. See INF-AUDIT-010.
- Staff role/group scoping and stale token rejection. See INF-AUDIT-011.
...
- Upgrade title: Unified guest status center
  ...
  - Complexity: Hard
  - Suggested wave: Wave 5
  - Dependencies: INF-AUDIT-006, INF-AUDIT-013, INF-AUDIT-048
...
- SSRF in image proxy and iCal sync: INF-AUDIT-004 and INF-AUDIT-012.
- Secrets/default credentials and dependency criticals: INF-AUDIT-005 and INF-AUDIT-016.

What should be fixed second:
- Public booking privacy, status-token leakage, email HTML injection, staff RBAC, inactive-user login, and admin auth/logout behavior.
...
- Start Wave 1 with INF-AUDIT-001, INF-AUDIT-002, INF-AUDIT-003, and INF-AUDIT-004 as a single security stabilization sprint, with regression tests added before or alongside each fix.
